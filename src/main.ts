...
function setFeedback(state: LocalGameState, tone: GameState["feedbackTone"], title: string, message: string): void {
...
export function toggleSelectedCard(state: LocalGameState, cardId: string): void {
	...
	if (!cardExists || state.phase !== "active_turn") {
		setFeedback(state, "illegal", "Illegal selection", "Only the active player can select cards during an active turn.");
		return;
	}
...
export function playMeld(state: LocalGameState): void {
	...
	if (state.selectedCardIds.length < 2) {
		setFeedback(state, "illegal", "Illegal meld", "Select at least two cards before playing a meld.");
		return;
	}
	...
	state.selectedCardIds = [];
	setFeedback(state, "legal", "Meld played", `${selectedCards.length} card${selectedCards.length === 1 ? "" : "s"} moved into the pathway.`);
	refreshAfterVisibleAction(state);
}
...
export function discardCard(state: LocalGameState): void {
	...
	state.selectedCardIds = [];
	setFeedback(state, "legal", "Card discarded", `${discardedCard.label} moved to the discard pile.`);
...
export function passScreen(state: LocalGameState): void {
	if (state.phase !== "active_turn") {
		setFeedback(state, "illegal", "Pass screen blocked", "Pass screen is available from an active turn.");
		return;
	}
...
export function startNextTurn(state: LocalGameState): void {
	...
	if (state.phase !== "pass_screen") {
		setFeedback(state, "illegal", "Next turn blocked", "Move to the pass screen before advancing the turn.");
		return;
	}
...
function createLocalGameController(state: LocalGameState, rerender: () => void): LocalGameController {
	const onDraw = (): void => {
		drawCard(state);
		rerender();
	};
