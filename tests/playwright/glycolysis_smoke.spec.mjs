...
async function main() {
	...
	try {
		...
		await assertText(page, ".controls__hint", "Draw a card, build a legal meld, then discard one card.");
		await assertVisible(page, 'button[title="Draw one card from the draw pile."]', "Draw tooltip should be present");
		...
		await assertText(page, ".feedback__message", "Select at least two cards before playing a meld.");
	} finally {
		await page.close();
		await browser.close();
		await new Promise((resolveServer) => {
